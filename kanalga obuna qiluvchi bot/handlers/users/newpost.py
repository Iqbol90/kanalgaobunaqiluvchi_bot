...
from aiogram.dispatcher import FSMContext  # holatlar-State()lar bilan ishlaganda foydalanamiz
...
async def enter_message(message: Message, state:FSMContext):
    # foydalanuvchi bizga matn yoki biror fayl yuboradi biz uni xotirada(state ni ichiga) saqlab quyishimiz kerak shuning uchun update_data() dan foydalanamiz
    await state.update_data(text=message.html_text, mention=message.from_user.get_mention())# mention bu foydalanuvchining ismi
    await message.answer(f"Postni tekshirish uchun yuboraymi", reply_markup=confirmation_keyboard)
    await NewPost.next() # keyingi state(holat) ga o'tadi
...
async def post_unknown(message: Message):
    await message.answer("Chop etish yoki Rda etishni tanlang")
